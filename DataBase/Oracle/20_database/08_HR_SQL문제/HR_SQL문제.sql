...
WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID
 ...
 ORDER BY NAME;
------------뷰(VIEW) 사용--------------
SELECT 'HR-Company', FIRST_NAME||' '||LAST_NAME NAME
       , JOB_ID, DEPARTMENT_NAME, CITY
FROM emp_details_view WHERE CITY = 'Oxford';
...
SELECT D.DEPARTMENT_NAME, SUB.CNT
  FROM   (SELECT DEPARTMENT_ID, COUNT(*) CNT
            FROM EMPLOYEES
           GROUP BY DEPARTMENT_ID
          HAVING COUNT(*) >=5) SUB, DEPARTMENTS D
 WHERE SUB.DEPARTMENT_ID = D.DEPARTMENT_ID
 ORDER BY 2 DESC;
---------- HR 유저에서 제공하는 VIEW 이용하여 처리
...
SELECT FIRST_NAME||' '||LAST_NAME, E.JOB_ID 
       , D.DEPARTMENT_NAME, E.HIRE_DATE, E.SALARY, G.GRADE_LEVEL
