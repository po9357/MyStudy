...
ROLLBACK TO 세이브포인트이름; --세이브 포인트 위치까지 취소
...
COMMIT;         -- 묵시적으로 SAVEPOINT 설정됨
-- DEPT1 테이블에서 ID : 40 데이터 삭제
...
-- ROLLBACK 세이브포인트;     -- 세이브포인트 지점까지 ROLLBACK
SELECT * FROM DEPT1;

-- S2까지 ROLLBACK
ROLLBACK TO S2;
SELECT * FROM DEPT1;        -- 10 총무부    복구됨

-- S1까지 ROLLBACK
ROLLBACK TO S1;
SELECT * FROM DEPT1;        -- 20 급여부    복구됨

-- 최종 COMMIT 시점까지 ROLLBACK
ROLLBACK;
SELECT * FROM DEPT1;        -- 40 인사부    복구됨