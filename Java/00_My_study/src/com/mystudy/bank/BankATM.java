...
class BankFuntion{
	...
	public void SignUp() {
		...
		while (true) {
			while (true) {
				...
				newPsw = sc.nextLine();
				Integer.parseInt(newPsw);
				} catch (NumberFormatException e) {
					System.out.println("숫자만 입력하여 주십시오.");
					break;
				}
				System.out.println("비밀 번호 4자리를 다시 입력하여 주십시오.");
				try {
				newPsw2 = sc.nextLine();
				Integer.parseInt(newPsw2);
				} catch (NumberFormatException e) {
					System.out.println();
				}
				if (newPsw.equals(newPsw2) && newPsw.toCharArray().length == newPsw2.toCharArray().length && newPsw.toCharArray().length == 4) {
					System.out.println("비밀 번호 입력이 완료되었습니다.");
					break Outter;
				} else 
					System.out.println("비밀번호 설정을 다시 시작합니다.");
			}
		}
	...
	public void AddAcc (String accNum, String name, String psw, int money) {
	...
	//비밀번호 조회
	public void ComparePsw(String psw) {
		int i;
		while (true) {
			try {
			for (i = 0; i < list.size(); i++) {
				if (psw.equals(list.get(i).getPsw())) {
