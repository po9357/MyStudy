...
-- ROWNUM : 오라클에서 제공하는 <가상의 ROW 번호>
-- 데이터가 조회순으로 값이 정해지는 형태
SELECT ROWNUM, B.* FROM BOOK B WHERE ROWNUM <= 5;
SELECT ROWNUM, B.* FROM BOOK B ORDER BY PRICE;  -- ROWNUM도 섞임. ROWNUM은 데이터를 조회한 순서
-- 정렬시 ROWNUM이 1 ~ N 오름차순 정렬이 깨짐

-- ROWNUM을 항상 1 ~ N 형태로 유지하려면
-- ORDER BY 적용 후 ROWNUM 조회
SELECT ROWNUM, B.*
  FROM (SELECT * FROM BOOK B ORDER BY PRICE) B -- ()안의 문장이 먼저 실행 (ORDER BY정렬 후 ROWNUM이 붙음)
 WHERE ROWNUM BETWEEN 1 AND 10;   -- 1부터는 가능. 중간만 잘라서 조회는 불가(2~4번째, 3~7번째 등)
 
SELECT *
FROM (SELECT ROWNUM RNO, B.*        -- ROWNUM을 컬럼 데이터화 하여
        FROM (SELECT * FROM BOOK    -- 중간만 잘라서 출력 가능
         ORDER BY PRICE) B
     )
...
CASE : ANSI 표준으로 모든 DB에서 사용가능(단, 지원안하면 사용 불가)

-- DECODE 함수
DECODE 함수는 동등비교만 가능
DECODE(대상, 비교값, 대상 = 비교값인 경우 처리문, 대상<>비교값인 경우 처리문)
...
SELECT NAME, DECODE(NAME, '김연아', '야~ 김연아다!!!', '누구세요?')
  FROM CUSTOMER
 ORDER BY NAME;
...
SELECT NAME, DECODE(NAME, '박지성', '축구선수', '운동선수')
  FROM CUSTOMER
 ORDER BY NAME;
-- 이름이 박지성이면 '축구선수', 김연아면 '피겨선수', 박세리면 '골프선수' 나머지 '운동선수'
-- 자바의 IF () {} ELSE IF () {} ... ELSE IF () {} ELSE {}
SELECT NAME, DECODE(NAME, '박지성', '축구선수'
                        , '김연아', '피겨선수'
                        , '박세리', '골프선수'
                        , '장미란', '역도선수'
                        , '추신수', '야구선수'
...
SELECT NAME, (CASE NAME
                   WHEN '박지성' THEN '축구선수' ELSE '운동성수' END) CASE_WHEN
  FROM CUSTOMER
 ORDER BY NAME;
-- 이름이 김연아 -> 피겨선수, 박세리 -> 골프선수, 박지성 -> 축구선수, 나머지 운동선수
SELECT NAME, (CASE NAME
                  WHEN '김연아' THEN '피겨선수'
                  WHEN '박세리' THEN '골프선수'
                  WHEN '박지성' THEN '축구선수'
                  ELSE '운동선수' END ) CASE_WHEN
  FROM CUSTOMER 
 ORDER BY NAME;
-------------------------
...
SELECT B.*, 
       CASE WHEN PRICE < 10000 THEN '싸다' ELSE
            CASE WHEN PRICE <= 20000 THEN '적당하다' ELSE
                 CASE WHEN PRICE <= 30000 THEN '비싸다' ELSE '너무비싸다' 
                 END 
            END
       END
  FROM BOOK B
 ORDER BY PRICE;
-- 추가 요청사항 발생 : 7000보다 적으면 '껌값이네' 추가
SELECT B.*, 
       (CASE WHEN PRICE < 7000 THEN '껌값이네' ELSE
             CASE WHEN PRICE < 10000 THEN '싸다' ELSE
                  CASE WHEN PRICE <= 20000 THEN '적당하다' ELSE
                       CASE WHEN PRICE <= 30000 THEN '비싸다' ELSE '너무비싸다' 
                       END 
 ...
 SELECT B.*, 
       (CASE WHEN PRICE < 10000 THEN CASE WHEN PRICE < 7000 
                                          THEN '껌값이네' ELSE '싸다' 
                                     END ELSE
             CASE WHEN PRICE <= 20000 THEN '적당하다' ELSE
                  CASE WHEN PRICE <= 30000 THEN '비싸다' ELSE '너무비싸다' 
