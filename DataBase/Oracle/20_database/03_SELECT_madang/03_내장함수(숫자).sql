...
--  FLOOR (숫자) : 소수점 아래 버림
SELECT FLOOR (30.999), FLOOR(30.11) FROM DUAL;          --  소수점 크기에 상관없이 버림

--  CEIL (숫자) : 올림
SELECT CEIL (30.999), CEIL (30.11), CEIL(30.0001) FROM DUAL;       --  소수점 이하가 존재하면 올림

--  ROUND (숫자) : 반올림  -   소수점 이하 반올림
--  ROUND (숫자, 자리수) : 자리수 이하 반올림    예)  ROUND (95.789, 2)   ->  95.79
SELECT ROUND (30.54), ROUND (30.49) FROM DUAL;
SELECT ROUND (30.55, 1) FROM DUAL;                              --  소수점 이하 '1'번 째 자리까지 표시 (2번째 자리에서 반올림)
SELECT ROUND (30.55, 1), ROUND (30.45, 1), ROUND (30.44, 1) FROM DUAL;

--  TRUNC(숫자) : 정수만 남기고, 소수부 버림
--  TRUNC(숫자, 자리수) : 정수 이하 자리수까지 유지 
SELECT TRUNC (30.54), TRUNC (30.49), TRUNC (30.999) FROM DUAL;          
SELECT TRUNC (30.55, 1), TRUNC (30.45, 1), ROUND (30.44, 1) FROM DUAL;

--  SIGN(숫자) : 부호값 반환(양수: 1, 영: 0, 음수: -1)
SELECT SIGN(0), SIGN(65), SIGN(-65) FROM DUAL;

--  POWER(숫자, n) : 숫자의 n제곱근 값 계산(POWER(2, 3) -> 8) 
