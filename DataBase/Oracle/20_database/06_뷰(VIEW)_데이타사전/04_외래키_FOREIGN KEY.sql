...
VALUES (1111, '홍길동', '직무01', 10);           -- 정상입력 처리
-- 오류 보고 - ORA-02291: integrity constraint (MADANG.SYS_C007030) violated - parent key not found
-- 부모테이블 (DEPT)의 ID컬럼에 40이라는 값이 없어서 오류 발생
INSERT INTO EMP01 VALUES(2222, '홍두께', '직무2', 40);
---------------------------------
-- 테이블 레벨 방식으로 제약조건 지정
CREATE TABLE EMP02 (
             EMPNO NUMBER,
...
INSERT INTO EMP02 VALUES (1111, '홍길동', '직무01', 10);
-- 오류 발생
INSERT INTO EMP02 VALUES (2222, '홍두께', '직무2', 40);
------------------------------------
...
INSERT INTO EMP03 VALUES (1111, '홍길동', '직무01', 10);
-- 오류 발생
INSERT INTO EMP03 VALUES (2222, '홍두께', '직무2', 40);
-------------------------------------
SELECT A.TABLE_NAME,
       A.COLUMN_NAME,
       A.CONSTRAINT_NAME,
...
-- 오류 보고 - ORA-00001: unique constraint (MADANG.HSCHOOL_HAK_BAN_BUN_PK) violated
INSERT INTO HSCHOOL VALUES (1,1,1, '강감찬2');
...
ALTER TABLE EMP01 DROP CONSTRAINT EMP01_PK;
INSERT INTO EMP01 VALUES (1111, '홍길동', '직무01', 10); -- 기본 키(PRIMARY KEY)를 삭제하면 중복데이터 입력 가능
SELECT * FROM EMP01;                                   --             (UNIQUE 제약조건도 없음)
-- EMP01 테이블에 PRIMARY KEY 추가
ALTER TABLE EMP01 ADD CONSTRAINT EMP01_EMPNO_PK PRIMARY KEY (EMPNO);
-- INSERT INTO EMP01 VALUES (1111, '홍길동', '직무01', 10);

-----------------------------------
-- EMP01 테이블의 외래키 (FOREIGN KEY) 삭제 후 추가
-- 외래키 SYS_C007030 삭제
ALTER TABLE EMP01 DROP CONSTRAINT SYS_C007030;
-- 외래키 추가 (EMP01_DEPTNO_FK)
ALTER TABLE EMP01 ADD CONSTRAINT EMP01_DEPTNO_FK FOREIGN KEY (DEPTNO) REFERENCES DEPT(ID);

-----------------------------------
/**************************************
제약조건을 임시적으로 활성화 또는 비활성화
-- 제약조건이 설정되어 있는 것을 적용해제 또는 적용
ALTER TABLE 테이블명 DISABLE CONSTRAINT 제약명;   -- 비활성화
ALTER TABLE 테이블명 ENABLE CONSTRAINT 제약명;    -- 활성화
**************************************/
