...
9(구) : 자리수를 나타내며, 자리수가 맞지 않는 경우 채우지 않음 (표시 안함)
L : 지역 통화 기호 표시
.(점) : 소수점
,(컴마) : 1000 단위 구분 기호
-----------------------------------------------*/
...
--  오라클에선 문자 타입도 자동 변환되어 연산이 가능하지만, 다른 DB에선 안되는 경우도 있다.
...
----------------------
--  TO_DATE (문자열, 문자열 형식 문자)
SELECT '2018-08-14', TO_DATE('2018-08-14', 'YYYY-MM-DD') FROM DUAL;
SELECT '2018-08-14  16:48:35', TO_DATE('2018-08-14  16:48:35', 'YYYY-MM-DD HH24:MI:SS') FROM DUAL;
-----------------------------
SELECT '0. 현재날짜 : ' || SYSDATE FROM DUAL
UNION
SELECT '1. 지금 : ' || TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') FROM DUAL
UNION
SELECT '2. 1일 : ' || TO_CHAR(SYSDATE + 1, 'YYYY-MM-DD HH24:MI:SS') FROM DUAL
UNION
...
SELECT 24 * 60, 24 * 60 * 60 FROM DUAL;         --  하루 24시간 = 1440분 = 86400초